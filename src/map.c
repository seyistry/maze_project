...
void renderOverheadMap() {
  ...
  for (row = 0; row < MAP_GRID_HEIGHT; row++) {
    for (col = 0; col < MAP_GRID_WIDTH; col++) {
      ...
      fillRect((int)(mapGridSquareSize * col) + mapXOffset,
               (int)(mapGridSquareSize * row) + mapYOffset, mapGridSquareSize,
               mapGridSquareSize);
    }
  }
  ...
  for (i = 0; i < WINDOW_WIDTH; i++) {
    ...
    if (fabs(homogeneousVectorMagnitude(&rays[i].hRay)) <
      ...
      ray = rays[i].vRay;
    drawLine(
        (int)(playerPos.x * HUD_MAP_SIZE / (float)MAP_PIXEL_WIDTH) + mapXOffset,
        (int)(playerPos.y * HUD_MAP_SIZE / (float)MAP_PIXEL_HEIGHT +
              mapYOffset),
        (int)((playerPos.x + ray.x) * HUD_MAP_SIZE / (float)MAP_PIXEL_WIDTH) +
            mapXOffset,
        (int)((playerPos.y + ray.y) * HUD_MAP_SIZE / (float)MAP_PIXEL_WIDTH) +
            mapYOffset);
    if (slowRenderMode) {
      ...
      drawLine((int)(playerPos.x * HUD_MAP_SIZE / (float)MAP_PIXEL_WIDTH) +
               ...
               (int)((playerPos.x + PLAYER_SIZE * playerDir.x) * HUD_MAP_SIZE /
                     (float)MAP_PIXEL_WIDTH) +
                   mapXOffset,
               (int)((playerPos.y + PLAYER_SIZE * playerDir.y) * HUD_MAP_SIZE /
                     (float)MAP_PIXEL_WIDTH) +
                   mapYOffset);
      setDrawColor(200, 100, 50, 255);
      SDL_Delay(2);
