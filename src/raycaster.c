...
void extendRaysToFirstHit(RayTuple *rays) {
  ...
  for (i = 0; i < VIEWPLANE_LENGTH; i++) {
    ...
    if (rays[i].vRay.x < 0) { /* Ray is facing left */
      perpVec.x =
          ((int)(playerPos.x / (float)WALL_SIZE)) * WALL_SIZE - playerPos.x;
    } else { /* Ray is facing right */
      perpVec.x = ((int)(playerPos.x / (float)WALL_SIZE)) * WALL_SIZE -
                  playerPos.x + WALL_SIZE;
    }
    rays[i].vRay = homogeneousVectorScale(
        &rays[i].vRay,
    ...
    if (rays[i].hRay.y < 0) { /* Ray is facing up */
      perpVec.y =
          ((int)(playerPos.y / (float)WALL_SIZE)) * WALL_SIZE - playerPos.y;
    } else { /* Ray is facing down */
      perpVec.y = ((int)(playerPos.y / (float)WALL_SIZE)) * WALL_SIZE -
                  playerPos.y + WALL_SIZE;
    }
    rays[i].hRay = homogeneousVectorScale(
        &rays[i].hRay,
        vectorDotProduct(&perpVec, &perpVec) /
...
void raycast(RayTuple *rays) {
  ...
  for (i = 0; i < VIEWPLANE_LENGTH; i++) {
    ...
    while (mapCoord.x > 0 && mapCoord.y > 0 && mapCoord.x < MAP_GRID_WIDTH &&
           mapCoord.y < MAP_GRID_HEIGHT &&
